import type { LocalUserChoices } from "@/lib/user-choices";
...
export function usePersistentUserChoices(
  ...
  const saveAudioOutputDeviceId = React.useCallback((deviceId: string) => {
    setSettings((prev) => ({ ...prev, audioOutputDeviceId: deviceId }));
  }, []);
